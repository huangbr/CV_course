...
def load_dataset_random(path, dataset, seed, task_type, tasks=None, random_num=None, logger=None):
    ...
    ''' 第一种划分：val->test->trn.    结果0.85 '''
    # val_id = indices[:val_size]
    # test_id = indices[val_size:(val_size+test_size)]
    # trn_id = indices[(val_size+test_size):]
    #
    ...
    # dataset_enhancement = pd.concat([val_df, test_df, trn_enhance_df],ignore_index=True)
    ...
    # val_ids = data_indices[:val_size]
    # test_ids = data_indices[val_size:(val_size+test_size)]
    # trn_ids = data_indices[(val_size+test_size):]
    #
    #
    ...
    #             f'train smiles = {len(trn_ids):,} | '
    #             f'val smiles = {len(val):,} | '
    #             f'test smiles = {len(test):,}')


    ''' 第二种划分: trn->val->test.   结果0.889'''
