...
def split_data(split_type: str = 'random',
               sizes: Tuple[float, float, float] = (0.8, 0.1, 0.1),
               seed: int = 0,
               args: Namespace = None,
               path: str = None) -> Tuple[list,
                                          list,
                                          list]:
    ...
    if split_type == 'random':
        ...
        train_val_size = int((sizes[1]+sizes[0]) * len(df))
        test_size = len(df) - train_val_size

        trn_idx = idx[:train_size]
        val_idx = idx[train_size:train_val_size]
        test_idx = idx[train_val_size:]
        ...
        # 数据增强
        trn_enhance_df = data_enhancement(trn_df,random_num = 1)
        ...
        trn_ids = list(range(len(trn_enhance_df)))
        val_ids = list(range(len(trn_enhance_df), len(trn_enhance_df)+len(val_df)))
        test_ids = list(range(len(trn_enhance_df)+len(val_df), len(trn_enhance_df)+len(val_df)+len(test_df)))

        # 保存扩增后的数据集
        # output_path = path + 'raw/' + dataset + '_enhancement.csv'
        output_path = 'data/{}_enhancement.csv'.format(args.dataset)
        output_hignn_path = 'HiGNN/data/{}/raw/{}_enhancement.csv'.format(args.dataset,args.dataset)
        dataset_enhancement.to_csv(output_path, index=False)
        dataset_enhancement.to_csv(output_hignn_path, index=False)
