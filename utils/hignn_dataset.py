...
from HiGNN.source.utils import get_task_names
...
class MolDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        df = pd.read_csv('HiGNN/data/{}/raw/{}_enhancement.csv'.format(self.dataset, self.dataset))
...
'''  构建HiGNN数据集 '''
def build_dataset_from_idx(cfg, logger, trn_idx, val_idx, test_idx):
    cfg.defrost()
    # task_name = get_task_names(os.path.join(cfg.DATA.DATA_PATH, 'raw/{}.csv'.format(cfg.DATA.DATASET)))
    task_name = get_task_names('HiGNN/data/{}/raw/{}_enhancement.csv'.format(cfg.DATA.DATASET, cfg.DATA.DATASET))
    ...
    trn, val, test = pyg_dataset[torch.LongTensor(trn_idx)], \
                     pyg_dataset[torch.LongTensor(val_idx)], \
                     pyg_dataset[torch.LongTensor(test_idx)]

    logger.info(f'Total smiles = {len(pyg_dataset):,} | '
                f'train smiles = {len(trn_idx):,} | '
                f'val smiles = {len(val_idx):,} | '
                f'test smiles = {len(test_idx):,}')
...
#     # `batch` 是一个包含 `(data, idx)` 对的列表
#     data, indices = zip(*[(item, i) for i, item in enumerate(batch)])
#     # 使用默认的 collate 来处理数据部分
...
def build_loader(cfg, logger, trn_idx, val_idx, test_idx, device):
    train_dataset, valid_dataset, test_dataset = build_dataset_from_idx(cfg, logger, trn_idx, val_idx, test_idx)
