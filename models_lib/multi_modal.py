...
from HiGNN.source.model import HiGNN
...
class Multi_modal(nn.Module):
    def __init__(self, args, cfg, compound_encoder_config, device):
        super().__init__()
        self.args = args
        self.device = device
        ...
        if args.pro_num == 3:
            self.pro_seq = (nn.Sequential(nn.Linear(args.seq_hidden_dim, self.latent_dim),
                                         nn.ReLU(inplace=True),
                                         nn.Linear(self.latent_dim, self.latent_dim))
                            .to(device))
            self.pro_gnn = (nn.Sequential(nn.Linear(cfg.MODEL.OUT_DIM, self.latent_dim),
                                         nn.ReLU(inplace=True),
                                         nn.Linear(self.latent_dim, self.latent_dim))
                            .to(device))
            self.pro_geo = (nn.Sequential(nn.Linear(args.geo_hidden_dim, self.latent_dim),
                                         nn.ReLU(inplace=True),
                                         nn.Linear(self.latent_dim, self.latent_dim))
                            .to(device))
        elif args.pro_num == 1:
        ...
        # Fusion
        fusion_dim = cfg.MODEL.HID * self.graph + args.seq_hidden_dim * self.sequence + \
    ...
    def forward(self, trans_batch_seq, seq_mask, batch_mask_seq, gnn_batch_graph, gnn_feature_batch, batch_mask_gnn,
                graph_dict, node_id_all, edge_id_all, train_dataset):

        x_list = list()
        cl_list = list()
        if self.graph:
            ''' HiGNN '''
            graph_gnn_x = self.gnn(train_dataset)
            if self.args.norm:
